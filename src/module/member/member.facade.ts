...
export class MemberFacade {
  ...
  async updateProfile(
    ...
    if (profileImage) {
      const existing = await this.fileFacade.findByFileTypeAndMemberId(
        FileTypes.Profile,
        uploader.memberId,
      );
      if (existing) {
    ...
    return this.toMemberResponse(
      await this.memberService.findMemberById(uploader.memberId),
    );
  }

  private async toMemberResponse(member: Member): Promise<MemberResponse> {
    const profileImage = await this.fileFacade.findByFileTypeAndMemberId(
      FileTypes.Profile,
      member.id,
    );
